...
前端不设置 multipart 的 `Content-Type`，由浏览器生成包含 boundary 的请求头。前端不解析 Excel，不传文件 URL、存储标识或对象存储路径；后端负责解析并把结果写入独立校验批次表和校验行表。

上传文件只作为 `VALIDATING` 期间的临时解析输入。后端进入 `PASSED`、`FAILED` 或 `SYSTEM_ERROR` 终态后清理临时文件；后续问题分页、导出、确认和 `RETRY_VALIDATION` 全部基于校验批次/行表，不需要保存原文件。
...
5. 视口为 1280 × 720 时，创建页“校验任务”完整位于默认收起的“智能体动态”浮层上方，普通点击可以触发。
6. 批量名单第一次校验失败，能看到行号、手机号、原因和重复行。
7. 下载问题明细后，替换上传完整名单，第二次校验通过。
8. 删除呼叫规则需要二次确认，删除后规则从列表和新建任务选项中消失。
9. 运行中任务平滑进入暂停中和已暂停，恢复后继续。
10. 停止任务先进入停止中，再进入已停止。
11. 定时任务只能修改名称和执行时间。
12. 任务详情只显示配置摘要和外呼对象，不显示三套 Tab。
13. 任务级和对象级按钮跳转统一通话记录并自动筛选。
14. 通话记录列表直接显示 AI 结论、摘要和录音入口。
15. AI Call 菜单中没有第一阶段数据看板。
