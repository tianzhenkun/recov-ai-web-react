...
> **历史快照，非现行规范。** 本文保留 2026-05-11 当时的迁移事实，不应用于指导当前开发或部署。当前业务请求只有主 API，`adminApi` 仅做正式 release 配置解析兼容且业务忽略；普通业务页面以 `/system/menu/getRouters` 的原始完整树（包括 `hidden` 路由）作为白名单。现行规则以项目 README、[前端模块规范](../前端模块规范.md) 和 [部署说明](../部署说明.md) 为准。
