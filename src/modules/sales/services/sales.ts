...
export type SalesRequirementType =
  | 'required'
  | 'excluded'
  | 'preferred'
  | string;
...
export type SalesGeneratedTemplateDraft = Omit<
  SalesIcpTemplate,
  'id' | 'raw' | 'status'
> & {
  status?: SalesTemplateStatus;
...
export type SalesProviderUsageSummaryRecord =
  SalesProviderUsageProviderRecord & {
...
export const normalizeTemplateItem = (
  ...
  return {
    ...
    scoreWeight: toNumber(
      readValue(record, ['scoreWeight', 'score_weight']),
      10,
    ),
    valueText,
...
const normalizeRequirement = (
  record?: SalesRequirement | null,
): SalesRequirement | null => {
  if (!record) return null;
  ...
  const text = toText(
    readValue(record, ['text', 'valueText', 'value_text', 'original']),
  );
  if (!text) return null;
...
export const normalizeSalesTask = (
  ...
  return {
    id: record.id,
    taskName: toText(
      readValue(record, ['taskName', 'task_name']),
      '未命名任务',
    ),
  ...
  return {
    ...
    progress:
      typeof record.progress === 'number'
        ? Math.max(0, Math.min(record.progress, 100))
        : undefined,
    callbackSeq: Number(
      readValue(record, ['callbackSeq', 'callback_seq']) || 0,
    ),
...
export const normalizeSalesSearchRunProgress = (
  ...
  return {
    ...
    taskErrorMessage: toText(
      readValue(record, ['taskErrorMessage', 'task_error_message']),
    ),
    ...
    currentStageLabel: toText(
      readValue(record, ['currentStageLabel', 'current_stage_label']),
    ),
    message: toText(record.message),
    errorMessage: toText(readValue(record, ['errorMessage', 'error_message'])),
    ...
    profileCount: toNumber(
      readValue(record, ['profileCount', 'profile_count']),
    ),
    sourceCount: toNumber(readValue(record, ['sourceCount', 'source_count'])),
    evidenceCount: toNumber(
      readValue(record, ['evidenceCount', 'evidence_count']),
    ),
    contactCount: toNumber(
      readValue(record, ['contactCount', 'contact_count']),
    ),
...
export const normalizeSalesAccountProfile = (
  ...
  if (!record?.id) return null;
  const primaryDomain = toText(
    readValue(record, ['primaryDomain', 'primary_domain']),
  );
  ...
  return {
    ...
    nameConfidence: toText(
      readValue(record, ['nameConfidence', 'name_confidence']),
    ),
    identityConfidence: toText(
      readValue(record, ['identityConfidence', 'identity_confidence']),
    ),
    ...
    sourceTypes: parseStringList(
      readValue(record, ['sourceTypes', 'source_types']),
    ),
    decision: toText(record.decision, 'uncertain'),
    decisionReason: toText(
      readValue(record, ['decisionReason', 'decision_reason']),
    ),
    fitScore: toNumber(readValue(record, ['fitScore', 'fit_score'])),
    reviewStatus: toText(
      readValue(record, ['reviewStatus', 'review_status']),
      'pending',
    ),
...
export const normalizeSalesLeadResult = (
  ...
  const profileId = readValue(record, ['profileId', 'profile_id']) as
    | SalesId
    | undefined;
  const primaryDomain = toText(
    readValue(record, ['primaryDomain', 'primary_domain']),
  ...
  return {
    ...
    taskName: toText(readValue(record, ['taskName', 'task_name'])),
    taskStatus: toText(
      readValue(record, ['taskStatus', 'task_status']),
      'completed',
    ),
    runId: readValue(record, ['runId', 'run_id']) as SalesId,
    runType: toText(readValue(record, ['runType', 'run_type'])),
    ...
    companyReviewNote: toText(
      readValue(record, ['companyReviewNote', 'company_review_note']),
    ),
    evidenceCount: toNumber(
      readValue(record, ['evidenceCount', 'evidence_count']),
    ),
    ...
    contactScore: toNumber(
      readValue(record, ['contactScore', 'contact_score']),
    ),
    ...
    contactReviewNote: toText(
      readValue(record, ['contactReviewNote', 'contact_review_note']),
    ),
    contactReason: toText(
      readValue(record, ['contactReason', 'contact_reason']),
    ),
    profile: toObject<Record<string, unknown>>(record.profile),
    createTime: toText(readValue(record, ['createTime', 'create_time'])),
    updateTime: toText(readValue(record, ['updateTime', 'update_time'])),
...
const normalizeSalesLeadEvidenceSummary = (
  ...
  supportedCount: toNumber(
    readValue(record, ['supportedCount', 'supported_count']),
  ),
  conflictedCount: toNumber(
    readValue(record, ['conflictedCount', 'conflicted_count']),
  ),
  unknownCount: toNumber(readValue(record, ['unknownCount', 'unknown_count'])),
  sourceCount: toNumber(readValue(record, ['sourceCount', 'source_count'])),
  requirementCount: toNumber(
    readValue(record, ['requirementCount', 'requirement_count']),
  ),
...
const normalizeSalesLeadRequirementCoverage = (
  ...
  if (!record) return null;
  return {
    requirementId: toText(
      readValue(record, ['requirementId', 'requirement_id']),
    ),
    requirementType: toText(
      readValue(record, ['requirementType', 'requirement_type']),
    ),
    requirementText: toText(
      readValue(record, ['requirementText', 'requirement_text']),
    ),
    attributeKey: toText(readValue(record, ['attributeKey', 'attribute_key'])),
    attributeName: toText(
      readValue(record, ['attributeName', 'attribute_name']),
    ),
    ...
    unknownCount: toNumber(
      readValue(record, ['unknownCount', 'unknown_count']),
    ),
  };
};
...
const normalizeSalesLeadEvidenceItem = (
  ...
  return {
    ...
    providerRank: toOptionalNumber(
      readValue(record, ['providerRank', 'provider_rank']),
    ),
...
const normalizeSalesLeadSourceItem = (
  ...
  return {
    ...
    matchedTerms: parseStringList(
      readValue(record, ['matchedTerms', 'matched_terms']),
    ),
    reason: toText(record.reason),
    resultId: readValue(record, ['resultId', 'result_id']) as SalesId,
    ...
    runProgress:
      normalizeSalesSearchRunProgress(
        readValue(record, [
          'runProgress',
          'run_progress',
        ]) as SalesSearchRunProgressRecord,
      ) ?? undefined,
    raw: record,
  };
};
...
export const normalizeSalesAccountChannel = (
  ...
  if (!record) return null;
  const channelValue = toText(
    readValue(record, ['channelValue', 'channel_value']),
  );
  ...
  return {
    ...
    channelType: toText(
      readValue(record, ['channelType', 'channel_type']),
      'other',
    ),
    channelValue,
    normalizedValue: toText(
      readValue(record, ['normalizedValue', 'normalized_value']),
    ),
    sourceUrl: toText(readValue(record, ['sourceUrl', 'source_url'])),
    sourceDomain: toText(readValue(record, ['sourceDomain', 'source_domain'])),
    confidence: toNumber(record.confidence),
    verifiedStatus: toText(
      readValue(record, ['verifiedStatus', 'verified_status']),
      'unverified',
    ),
    reason: toText(record.reason),
...
export const normalizeSalesAccountContactEvidence = (
  ...
  return {
    ...
    evidenceKey: toText(readValue(record, ['evidenceKey', 'evidence_key'])),
    evidenceType: toText(
      readValue(record, ['evidenceType', 'evidence_type']),
      'identity',
    ),
    judgment: toText(record.judgment, 'unknown'),
    evidenceText: toText(readValue(record, ['evidenceText', 'evidence_text'])),
...
export const normalizeSalesProviderUsageProvider = (
  ...
  cacheHitCount: toNumber(
    readValue(record, ['cacheHitCount', 'cache_hit_count']),
  ),
...
export const normalizeSalesAccountContacts = (
  ...
  return {
    ...
    summary: {
      ...
      contactCount: toNumber(
        readValue(summary, ['contactCount', 'contact_count']),
      ),
      contactChannelCount: toNumber(
        readValue(summary, ['contactChannelCount', 'contact_channel_count']),
      ),
      evidenceCount: toNumber(
        readValue(summary, ['evidenceCount', 'evidence_count']),
      ),
    },
...
export const normalizeSalesEmailAccount = (
  ...
  if (!record?.id) return null;
  const emailAddress = toText(
    readValue(record, ['emailAddress', 'email_address']),
  );
  const accountName = toText(
    readValue(record, ['accountName', 'account_name']),
    emailAddress,
  );
  ...
  return {
    ...
    providerLabel: toText(
      readValue(record, ['providerLabel', 'provider_label']),
    ),
    smtpHost: toText(readValue(record, ['smtpHost', 'smtp_host'])),
    smtpPort: toNumber(readValue(record, ['smtpPort', 'smtp_port']), 465),
    smtpEncryption: toText(
      readValue(record, ['smtpEncryption', 'smtp_encryption']),
      'SSL_TLS',
    ),
    ...
    defaultFlag: toText(
      readValue(record, ['defaultFlag', 'default_flag']),
      '0',
    ),
    ...
    smtpTestMessage: toText(
      readValue(record, ['smtpTestMessage', 'smtp_test_message']),
    ),
    ...
    imapTestMessage: toText(
      readValue(record, ['imapTestMessage', 'imap_test_message']),
    ),
...
export const normalizeSalesEmailMessage = (
  ...
  return {
    ...
    profileId: readValue(record, ['profileId', 'profile_id']) as SalesId,
    contactId: readValue(record, ['contactId', 'contact_id']) as SalesId,
    contactChannelId: readValue(record, [
      'contactChannelId',
      'contact_channel_id',
    ]) as SalesId,
    toEmail,
    normalizedToEmail: toText(
      readValue(record, ['normalizedToEmail', 'normalized_to_email']),
    ),
    ...
    contentFormat: toText(
      readValue(record, ['contentFormat', 'content_format']),
      'plain',
    ),
    status: toText(record.status, 'draft'),
    providerMessageId: toText(
      readValue(record, ['providerMessageId', 'provider_message_id']),
    ),
    messageIdHeader: toText(
      readValue(record, ['messageIdHeader', 'message_id_header']),
    ),
    ...
    lastAttemptAt: toText(
      readValue(record, ['lastAttemptAt', 'last_attempt_at']),
    ),
...
export const normalizeSalesEmailEvent = (
  ...
  return {
    ...
    accountId: readValue(record, ['accountId', 'account_id']) as SalesId,
    eventType: toText(
      readValue(record, ['eventType', 'event_type']),
      'unknown',
    ),
...
export const normalizeSalesEmailDoNotContact = (
  ...
  return {
    ...
    ownerUserId: readValue(record, ['ownerUserId', 'owner_user_id']) as SalesId,
    email,
    normalizedEmail: toText(
      readValue(record, ['normalizedEmail', 'normalized_email']),
    ),
    reasonType: toText(
      readValue(record, ['reasonType', 'reason_type']),
      'manual',
    ),
    reason: toText(record.reason),
    sourceMessageId: readValue(record, [
      'sourceMessageId',
      'source_message_id',
    ]) as SalesId,
    createTime: toText(readValue(record, ['createTime', 'create_time'])),
...
const toRequirementPayload = (item: SalesRequirement) => ({
  ...
  attrSource:
    item.attrSource ||
    item.attr_source ||
    (item.attributeKey || item.attribute_key ? 'system' : 'custom'),
...
export const generateSalesTemplate = async (
  data: GenerateSalesTemplatePayload,
) => {
  const response = await ruoyiRequest<SalesIcpTemplateRecord>(
...
export const deleteSalesTemplates = (ids: SalesId[]) =>
  ruoyiRequest<void>(
    `${SALES_JAVA_API_PREFIX}/icp/templates/${ids.join(',')}`,
    {
      method: 'delete',
    },
  );
...
export const querySalesLeadResultPage = async (
  params?: SalesLeadResultQuery,
) => {
...
export const saveSalesEmailAccount = async (data: SalesEmailAccountPayload) => {
  const response = await ruoyiRequest<SalesEmailAccountRecord>(
...
export const createSalesEmailDraft = async (data: SalesEmailMessagePayload) => {
  const response = await ruoyiRequest<SalesEmailMessageRecord>(
...
export const startSalesTask = (taskId: SalesId) =>
  ruoyiRequest<SalesTaskStartRecord>(
    `${SALES_JAVA_API_PREFIX}/tasks/${taskId}/start`,
    {
      method: 'post',
    },
  );
