...
export type LingchenRuntimeConfig = {
  ...
  /** @deprecated Formal release compatibility only; business requests ignore it. */
...
const readSameOriginPath = (config: UnknownRecord, key: string) => {
  ...
  try {
    assertSafeSameOriginPath(value, key);
  } catch {
...
const resolveEffectiveSsePath = (baseApi: string, ssePath: string) => {
  if (ssePath === baseApi || ssePath.startsWith(`${baseApi}/`)) {
    return ssePath;
  }
  return `${baseApi}${ssePath}`;
};
...
export const parseRuntimeConfig = (value: unknown): LingchenRuntimeConfig => {
  ...
  if (baseApi === '/' || adminApi === '/' || ssePath === '/') {
    fail('API channels must not use the site root.');
  }
  const apiChannels = [baseApi, adminApi].filter((item): item is string =>
    Boolean(item),
  );
  if (new Set(apiChannels).size !== apiChannels.length) {
    fail('baseApi and legacy adminApi must use different paths.');
  }
  const effectiveSsePath = resolveEffectiveSsePath(baseApi, ssePath);
  if (apiChannels.includes(effectiveSsePath)) {
    fail('effective SSE route must not conflict with API channels.');
  }
