...
import { parseSiteProfile } from '@/site-profiles';
import type {
  LoginFormValues,
  LoginInitErrors,
  LoginLayoutProps,
...
export const useLoginController = () => {
  ...
  const initSite = useCallback(async () => {
    ...
    try {
      ...
      const nextSiteConfig = parseSiteProfile(response.data);
      setSiteConfig(nextSiteConfig);
      setInitialState((state) => ({
        ...state,
        siteProfile: nextSiteConfig,
      }));
  ...
  }, [form, initTenantList, setInitialState]);
