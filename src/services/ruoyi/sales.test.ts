...
describe('sales service run normalization', () => {
  ...
  it('queries flattened sales lead task results from Java', async () => {
    ...
    expect(ruoyiRequest).toHaveBeenCalledWith('/system/sales/leads/results/page', {
      method: 'get',
      params: {
        pageNum: 1,
        pageSize: 10,
      ...
      company_channels: [
        {
          ...
          confidence: 55,
          verified_status: 'format_valid',
          reason: '疑似个人邮箱，但未发现明确姓名或职位，先作为公司待归属渠道保留',
          raw: {
            ownerScope: 'company',
  ...
  it('normalizes and queries sales task progress', async () => {
    ...
    expect(ruoyiRequest).toHaveBeenCalledWith('/system/sales/tasks/2001/progress', {
      method: 'get',
    });
